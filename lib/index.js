...
(function () {
  var YLApp = {
     ...
     * */
    emit: function emit(event, data, target) {
    ...
    eval: function _eval(method, data, cb) {
      var session = this._idCounter++;
    ...
    hashBugForIeFix: function hashBugForIeFix() {
      document.body.focus(); //hash的bug，兼容IE
    },
    getWinObject: function getWinObject(id) {
      //获取同域的子窗体句柄
      try {
  ...
  var ylOnMessage = function ylOnMessage(message) {
      ...
      case "ylui-ping":
        if (YLApp.id) {
          ...
          if (YLApp._cbReady) {
            ...
            if (relCbReady !== false) YLApp.emit('ready', null, true); //发送ready事件
          }
          if (!YLApp._created) {
  ...
  try {
    module.exports = YLApp;
  } catch (e) {
    if (window.YLApp === undefined) {
      window.YLApp = YLApp;
    }
  }
})();