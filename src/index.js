(() => {
  const YLApp = {
    ...
    getWinObject: function (id) {
      //获取同域的子窗体句柄
      try {
        const win = parent.YL.vue.wins[id];
        const idIframe = win.idIframe;
        const iframe = parent.document.getElementById(idIframe);
  ...
  const ylOnMessage = function (message) {
